...
void ErrorHelp(char cmd[50]) { //{{{
  ...
  fprintf(stderr, "   %s <input.vcf> <input.agg> <width> <output.txt> <agg sizes> <options>\n\n", cmd);
...
system.\n\n"); */
      ...
      fprintf(stdout, "   %s <input.vcf> <input.agg> <width> <output.txt> <agg sizes> <options>\n\n", argv[0]);
...
//      printf("\nAggregate[%d]\n", i);
        ...
        for (int j = 1; j < bins; j++) {
          ...
          // start calculations from distance 0.5
          if (r > 0.1) {
...
//          N = 1000;
            ...
            elstat_potential_sqr[correct_size][j] += SQR(temp_elstat[j]) / N_count;
//          printf("j = %d; dist = %lf; N_count = %d; N = %d\n", j, j*width, N_count, N);
          }
        }
...
//        if (MoleculeType[moltype].Use && BeadType[Bead[bead].Type].Charge != 0) {
//          Vector dist = Distance(Bead[bead].Position, com, BoxLength);
//          dist.x = sqrt(SQR(dist.x) + SQR(dist.y) + SQR(dist.z));

//          if (dist.x < (width*bins)) {
//            int k = dist.x / width;

//            for (int l = 0; l < Counts.Beads; l++) {
//              if (BeadType[Bead[l].Type].Charge != 0) {
//                dist = Distance(Bead[bead].Position, Bead[l].Position, BoxLength);
//                double rij = sqrt(SQR(dist.x) + SQR(dist.y) + SQR(dist.z));
//                double coulomb;

//                if (bead != l) {
//                  coulomb = bjerrum * BeadType[Bead[bead].Type].Charge * BeadType[Bead[l].Type].Charge / rij;
//                  if (rij < r_c) { // short ranged part
//                    temp_elstat[k] += coulomb * (1 - (1 + beta) * exp(-2 * beta * rij));
//                  } else { // long ranged part
//                    temp_elstat[k] += coulomb;
//                  }
//                }
...
//      // monomer aggregate beads //{{{
//      for (int j = 0; j < Aggregate[i].nMonomers; j++) {
//        int bead = index[Aggregate[i].Monomer[j]];
//        printf("%d: %d (%d) %s\n", Aggregate[i].nMonomers, Aggregate[i].Monomer[j], bead, BeadType[Bead[bead].Type].Name);

//        if (BeadType[Bead[bead].Type].Charge != 0) {
//          Vector dist = Distance(Bead[bead].Position, com, BoxLength);
//          dist.x = sqrt(SQR(dist.x) + SQR(dist.y) + SQR(dist.z));
...
//      // add from temporary elstat array to global elstat array //{{{
//      for (int j = 0; j < bins; j++) {
//        elstat[correct_size][j] += temp_elstat[j];
//      } //}}}
//    } //}}}
      ...
      if ((agg_sizes[i][1] % 100) == 0) {
        ...
        for (int j = 0; j < Counts.TypesOfMolecules; j++) {
          if (specific_moltype_for_size[j]) {
            char str2[256];
            sprintf(str2, "%s-%d-%s", str, agg_sizes[i][1], MoleculeType[j].Name);
            strcpy(str, str2);
          }
        }
        char str2[256];
        ...
        for (int j = 0; j < bins; j++) {
          if ((width*j) > 0.1) {
            ...
            double stdev = sqrt((elstat_potential_sqr[i][j] - SQR(elstat_potential[i][j])) / agg_sizes[i][1]);
            fprintf(out, " %20.15f %20.15f", elstat_potential[i][j]/agg_sizes[i][1], stdev);
            putc('\n',out);
          }
        }
  ...
  for (int i = 0; i < aggs; i++) {
    ...
    for (int j = 0; j < bins; j++) {
      if ((width*j) > 0.1) {
        ...
        double stdev = sqrt((elstat_potential_sqr[i][j] - SQR(elstat_potential[i][j])) / agg_sizes[i][1]);
        fprintf(out, " %20.15f %20.15f", elstat_potential[i][j]/agg_sizes[i][1], stdev);
//      fprintf(out, " %20.15f", elstat[i][j]/agg_sizes[i][1]);
        putc('\n',out);
      }
    }
