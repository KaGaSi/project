...
//fprintf(out, "\n# for each molecule type: rdp | stderr | rnp | stderr\n"); //}}}
  ...
  while ((test = getc(vcf)) != EOF) {
    ...
    for (int i = 0; i < Counts.Beads; i++) {
      ...
      if (use) {
        // here be dragons
        // 1) define bead to check
        // 2) check if the bead coordinate (x, y, or z) is proper (consider both walls)
        // 3) if not to be calculated, 'continue;'

        // 1) check last bead against the first bead
        if (mol != -1) {
          int mtype = Molecule[mol].Type;
          int bead1 = Molecule[mol].Bead[0];
          int bead2 = Molecule[mol].Bead[MoleculeType[mtype].nBeads-1];
          // 2) condition: first bead is closer to wall than the last one
          // 3) continue if condition not met
          if (axis == 'x') {
            double pos1 = Bead[bead1].Position.x,
                   pos2 = Bead[bead2].Position.x;
            if ((pos1 < (BoxLength.x/2) && pos1 < pos2 && fabs(pos1-pos2) < 1) || // 1st wall
                (pos1 > (BoxLength.x/2) && pos1 > pos2 && fabs(pos1-pos1) < 1)) { // 2nd wall
              continue;
            }
          } else if (axis == 'y') {
            double pos1 = Bead[bead1].Position.y,
                   pos2 = Bead[bead2].Position.y;
            if ((pos1 < (BoxLength.y/2) && pos1 < pos2 && fabs(pos1-pos2) < 1) || // 1st wall
                (pos1 > (BoxLength.y/2) && pos1 > pos2 && fabs(pos1-pos1) < 1)) { // 2nd wall
              continue;
            }
          } else {
            double pos1 = Bead[bead1].Position.z,
                   pos2 = Bead[bead2].Position.z;
            int ex = 2;
            double end_to_end = 1;
            if (ex == 0) { // check only molecules with end beads relatively horizontal with wall
              if ((pos1 < (BoxLength.z/2) && (fabs(pos1-pos2) >= end_to_end)) || // 1st wall
                  (pos1 > (BoxLength.z/2) && (fabs(pos1-pos1) >= end_to_end))) { // 2nd wall
                continue;
              }
            } else if (ex == 1) { // check only molecules with 1st bead nearer wall (and not too horizontal with wall)
              if ((pos1 < (BoxLength.z/2) && (pos1 < pos2 || fabs(pos1-pos2) < end_to_end)) || // 1st wall
                  (pos1 > (BoxLength.z/2) && (pos1 > pos2 || fabs(pos1-pos2) < end_to_end))) { // 2nd wall
                continue;
              }
            } else { // check only molecules with 2nd bead nearer wall (and not too horizontal with wall)
              if ((pos1 < (BoxLength.z/2) && (pos1 > pos2 || fabs(pos1-pos2) < end_to_end)) || // 1st wall
                  (pos1 > (BoxLength.z/2) && (pos1 < pos2 || fabs(pos1-pos2) < end_to_end))) { // 2nd wall
                continue;
              }
